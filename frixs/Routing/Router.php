...
class Router
{
    ...
    public static function redirectTo($path, $withoutAppRoot = false, $status = 301)
    {
        ...
        if (!$withoutAppRoot) {
            $path = Config::get('app.root_rel') .'/'. $path;
        }
    ...
    public static function redirectToError($errorCode = 500, $message = 'null')
    {
        ...
        switch ($errorCode) {
            case 001: // Maintenance
                header('Location: '. Config::get('app.root_rel') .'/error/001', true, 301);
                exit();
                break;
            case 401:
                header('Location: '. Config::get('app.root_rel') .'/error/401', true, 301);
                exit();
                break;
            case 404:
                header('HTTP/1.1 404 Not Found');
                header('Location: '. Config::get('app.root_rel') .'/error/404', true, 301);
                exit();
                break;
            case 500:
                header('Location: '. Config::get('app.root_rel') .'/error/500', true, 301);
                exit();
                break;
            case 501:
                header('Location: '. Config::get('app.root_rel') .'/error/501', true, 301);
                exit();
                break;
            case 503:
                header('Location: '. Config::get('app.root_rel') .'/error/503', true, 301);
                exit();
                break;
            default:
