...
class App
{
    ...
    {
        // TODO separate this logic to Router & Middleware
        $url = $this->parseUrl();

        // check if requested controller exists
        if (file_exists('../app/Http/Controllers/Pages/'. $url[0] . $this->controllerPostfix .'.php')) {
            $this->controller = $url[0];
            unset($url[0]);
        }
        
        $controllerFullName = '\\App\\Http\\Controllers\\Pages\\'. $this->convertToStudlyCaps($this->controller) . $this->controllerPostfix;
